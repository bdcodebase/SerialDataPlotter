...
import sys
import pandas as pd
from PyQt5.QtWidgets import  QMainWindow, QFileDialog, QAction, QInputDialog, QWidget, QVBoxLayout
...
class CSVPlotter(QMainWindow):
    ...
    def plotCSV(self, fileName):
        ...
        if all(isinstance(col, str) for col in data.columns):
            # Check if the first column should be used as the x-axis
            use_first_column_as_x = QtWidgets.QMessageBox.question(
                self, 'Use First Column as X-Axis', 'Do you want to use the first column as the x-axis?',
                QtWidgets.QMessageBox.Yes | QtWidgets.QMessageBox.No
            ) == QtWidgets.QMessageBox.Yes

            if use_first_column_as_x:
            ...
            for i, column in enumerate(data.columns):
                ...
                if i == len(data.columns) - 1:
                    plotWidget.setLabel('bottom', 'Samples' if not use_first_column_as_x else data.columns[0], color=self.foreground_color)
